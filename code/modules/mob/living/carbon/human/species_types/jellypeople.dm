...
/datum/species/jelly/proc/cannibalize_body(mob/living/carbon/human/target)
	...
	target.adjust_blood_volume(65 * GET_PHYSIOLOGY(target, PHYS_COEFF_BLOOD_REGEN)) // NOVA EDIT CHANGE - This is because losing a limb now costs 60 blood, so this refunds it with a bit extra; ORIGINAL: target.adjust_blood_volume(20 * GET_PHYSIOLOGY(target, PHYS_COEFF_BLOOD_REGEN))

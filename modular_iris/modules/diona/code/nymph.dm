...
/mob/living/basic/nymph/get_stat_tab_status()
	var/list/tab_data = ..()
...
/mob/living/basic/nymph/Life(delta_time, times_fired)
	. = ..()
	if(!is_drone)
		update_progression()
	if(stat != CONSCIOUS)
...
/mob/living/basic/nymph/death(gibbed)
...
/mob/living/basic/nymph/adjustBruteLoss(amount, updating_health, forced)
	brute_damage = brute_damage + amount * damage_coeff[BRUTE] * CONFIG_GET(number/damage_multiplier)
	. = ..()

/mob/living/basic/nymph/adjustFireLoss(amount, updating_health, forced)
	fire_damage = fire_damage + amount * damage_coeff[BURN] * CONFIG_GET(number/damage_multiplier)
	. = ..()

/mob/living/basic/nymph/UnarmedAttack(atom/A, proximity)
	melee_damage = 1.5
	. = ..()

/mob/living/basic/nymph/attack_animal(mob/living/L)
	if(istype(L, /mob/living/basic/nymph) && stat != DEAD)
		var/mob/living/basic/nymph/user = L
...
/mob/living/basic/nymph/spawn_gibs()
	new /obj/effect/decal/cleanable/insectguts(drop_location())
	playsound(drop_location(), 'sound/effects/blob/blobattack.ogg', 60, TRUE)

/mob/living/basic/nymph/attack_ghost(mob/dead/observer/user)
	if(client || key || ckey)
	...
	if(QDELETED(src) || QDELETED(user) || !user.client)
		return
	var/mob/living/basic/nymph/newnymph = src
...
/mob/living/basic/nymph/proc/update_progression()
...
/mob/living/basic/nymph/proc/nymph_assimilation(datum/source, atom/movable/arrived)
	if(!isdiona(arrived))
		return
...
/mob/living/basic/nymph/proc/evolve(mob/living/simple_animal/hostile/retaliate/nymph/nymphs)
	...
	var/mob/living/basic/nymph/helpers
	if(!nymphs)
		helpers = new /mob/living/basic/nymph(src.loc)
	else
		helpers = nymphs
...
/mob/living/basic/nymph/say(message, bubble_type, list/spans = list(), sanitize = TRUE, datum/language/language, ignore_spam = FALSE, forced)
	if(!..())
		emote("chitter")
...
/mob/living/basic/nymph/mob_try_pickup(mob/living/user)
	if(!ishuman(user))
		return
...
/mob/living/basic/nymph/mob_pickup(mob/living/L)
	if(resting)
		resting = FALSE
		update_resting()
	toggle_ai(AI_OFF)
...
/obj/item/clothing/head/mob_holder/nymph/release()
	on_head = FALSE
	var/mob/living/basic/nymph/nymph_mob = held_mob
